...
// OpenAI-compatible endpoint. Model used: meta-llama/llama-3.3-70b-instruct:free
// ($0 — a free-tier model, good multilingual support incl. German).
//
// IMPORTANT — free model caveats:
// - Rate limited (roughly 20 requests/minute, and a daily cap that's much
//   lower until you've bought $10+ of OpenRouter credits — check current
//   limits at openrouter.ai/docs).
// - Free ":free" model IDs rotate and get delisted by providers without
//   notice. Before deploying, verify this exact ID is still live at
//   openrouter.ai/models (filter by price, ascending) and have a fallback
//   paid model ready in case it disappears.
// - Not recommended for production traffic you depend on — fine for
//   low-volume, non-critical, or testing use.
//
// SETUP:
...
// Fallback message shown when the free model fails, is rate-limited, or
// returns something unusable (empty reply, non-string, etc). This keeps
// the widget from ever showing a raw error or a blank bubble to a visitor.
const FALLBACK_REPLY =
  "Entschuldigung, gerade komme ich kurz nicht durch. Bitte versuchen Sie es in einem Moment erneut oder schreiben Sie uns direkt an moin@kompasslotse.de.";

// Safety net: some upstream providers occasionally leak internal
// moderation/classifier tags (e.g. "User Safety: safe Response Safety: safe")
// into the visible reply text. Strip anything that looks like that before
// it ever reaches the browser.
function sanitizeReply(text) {
  if (typeof text !== "string") return text;
  return text
    // Remove lines/fragments like "User Safety: safe Response Safety: safe"
    .replace(/\b(User|Response)\s*Safety:\s*\w+\b/gi, "")
    // Collapse leftover double spaces / empty lines created by the removal
    .replace(/[ \t]{2,}/g, " ")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}
...
module.exports = async function handler(req, res) {
  ...
  if (!apiKey) {
    ...
    // Still return 200 with a friendly fallback so the widget never shows
    // a raw error to a visitor — just log the real cause server-side.
  ...
  try {
    ...
    // Free models on OpenRouter commonly fail with 429 (rate limit), 502/503
    // (provider overloaded), or return malformed JSON. Catch all of that
    // here and hand back a friendly fallback instead of surfacing it.
    if (!upstream.ok || !data) {
    ...
    // Free-tier models occasionally return an empty string, null, or a
    // non-string content array instead of plain text — guard against all
    // of that rather than sending a blank/broken bubble to the visitor.
    if (typeof reply !== "string" || reply.trim() === "") {
      console.error("OpenRouter returned an unusable reply:", JSON.stringify(data));
      return res.status(200).json({ reply: FALLBACK_REPLY });
    }
    ...
    */
    ...
    // Network failure, timeout, etc. — same friendly fallback.
