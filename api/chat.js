...
// OpenAI-compatible endpoint. Model used: nvidia/nemotron-3-ultra-550b-a55b:free
// ($0 — a free-tier, open-weight reasoning model from NVIDIA: a
// Mixture-of-Experts model (55B active / 550B total params, hybrid
// Transformer-Mamba) with a 1M token context window, built for
// multi-step reasoning and agentic workflows — which should help with
// staying on-script for the pricing/offer details in the system prompt.
// German quality is not specifically confirmed — worth testing. It's
// also a reasoning model, so part of its output budget can go to
// internal reasoning before the visible reply — worth checking that
// max_tokens below (currently 400) is comfortably enough in practice,
// and raising it if replies ever come back cut off. Still subject to
// free-tier deprioritization under load — see caveats below.)
//
// IMPORTANT — free model caveats:
// - Rate limited (roughly 20 requests/minute, and a daily cap that's much
//   lower until you've bought $10+ of OpenRouter credits).
// - Free requests are generally deprioritized behind paid requests on
//   OpenRouter's infrastructure, which can mean slower responses during
//   busy periods — this is true across most free models, not just this
//   one.
...
//   ready in case it disappears.
// - Not recommended for production traffic you depend on — fine for
//   low-volume, non-critical, or testing use. If this also falls short,
//   the paid anthropic/claude-haiku-4.5 is very cheap ($1 input / $5
//   output per million tokens), isn't deprioritized, and is meaningfully
//   more capable at following the detailed system prompt.
//
// SETUP:
