...
// OpenAI-compatible endpoint. Model used: inclusionai/ling-3.0-tiny:free
// ($0 — a free-tier model. Very small MoE model, ~1.3B active params out of
// 7.9B total, so it should be fast. This is the smallest model tried so far
// on this project — worth watching closely for whether it reliably follows
// the detailed system prompt (pricing tiers, guardrails, sentence-length
// limit) rather than just being fast. Still subject to free-tier
// deprioritization under load — see caveats below.)
//
...
//   low-volume, non-critical, or testing use. If this also fails, the
//   paid anthropic/claude-haiku-4.5 is very cheap ($1 input / $5 output
//   per million tokens) and isn't deprioritized.
// - Fallback free options if this ID gets delisted, or if quality turns out
//   too weak: google/gemma-4-26b-a4b-it:free or openai/gpt-oss-20b:free
//   (both bigger/slower but more reliable at following detailed system
//   prompts — check openrouter.ai/models before relying on any of these,
//   since free IDs rotate without notice).
//
// SETUP:
