  ...
  try {
    const upstream = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        // Pin the request to Anthropic's own endpoint instead of letting
        // OpenRouter load-balance across third-party providers. Some of those
        // third-party routes (e.g. Bedrock/Vertex) run their own moderation
        // wrapper in front of the model, and every once in a while that
        // wrapper's raw safety classification ("User Safety: safe / Response
        // Safety: safe") leaks through as the message content instead of the
        // actual reply. Sticking to Anthropic directly avoids that wrapper.
        provider: {
          order: ["Anthropic"],
    ...
    // Belt-and-braces filter: if a moderation/guardrail classifier's raw
    // output ever slips through anyway (e.g. via a fallback provider),
    // don't show it to the visitor - treat it like a failed request instead.
    const looksLikeSafetyClassifierOutput =
      typeof reply === "string" &&
      /\b(user|response)\s+safety\s*:/i.test(reply);

    if (!reply || !reply.trim() || looksLikeSafetyClassifierOutput) {
      console.error(
        "Suppressed non-answer from upstream (likely a moderation wrapper artifact):",
        JSON.stringify(data)
      );
      reply = null;
    }
